...
**Examples:**
```
Cloud AI: Voice → Internet → Server → Response → Internet → Device
On-Device: Voice → Local Model → Response (instantly)
```
...
- **Ultra-low latency** - No network round trips (< 100ms vs 1-5 seconds)
...
- **Smaller models = lower accuracy** - GPT-4 vs mobile-optimized models
...
- **Apple A17/M-series** - 15.8 TOPS Neural Engine
...
- But too heavy for mobile deployment
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Python Side   │ →  │  Serialization  │ →  │   C++ Runtime   │
│                 │    │                 │    │                 │
│ • Model Export  │    │ • .pte Format   │    │ • Inference     │
│ • Optimization  │    │ • Model Graph   │    │ • Memory Mgmt   │
│ • Quantization  │    │ • Weights       │    │ • Hardware Acc  │
└─────────────────┘    └─────────────────┘    └─────────────────┘

    Developer           Deployment Package     Production Device
```
...
- **Clean separation** - Export once, run anywhere
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Your Model    │ →  │  ExecuTorch     │ →  │    Hardware     │
│                 │    │    Runtime      │    │    Backend      │
│ • .pte file     │    │                 │    │                 │
│ • Operators     │    │ • Scheduling    │    │ • CPU (default) │
│ • Tensors       │    │ • Memory Mgmt   │    │ • XNNPACK       │
└─────────────────┘    └─────────────────┘    │ • CoreML        │
                                              │ • Qualcomm      │
                                              │ • Others...     │
                                              └─────────────────┘
...
## Setup
- Clone the repository: [https://github.com/NorbertKlockiewicz/rtc_on](https://github.com/NorbertKlockiewicz/rtc_on)
...
![](images/optimum-logo.png){width="100%"}
...
Expected Result: `128256` -- generated token
...
- __Transformer architecture__ - processe text in parallel, not sequentially
...
Input: "The cat sat on the"
       ↓ Self-Attention
"The" looks at: [cat, sat, on, the] - learns relationships
"cat" looks at: [The, sat, on, the] - understands it's the subject  
"sat" looks at: [The, cat, on, the] - learns it's an action
```
\

__Result__: Each word understands its context relative to all other words
...
├── simple_token_generator.h    # Token generation interface
├── simple_token_generator.cpp  # Generation loop
...
public:
  ...
  std::string decode(uint64_t prev_token, uint64_t cur_token);
...
class TextPrefiller {
public:
  TextPrefiller(executorch::extension::Module *module, bool use_kv_cache);
...
// 2. Prefill phase
auto prefill_res = text_prefiller_->prefill(prompt_tokens,
pos);
...
# Step 7: Speech-to-Text with Whisper
...
**The Process:**
```
Audio Waveform → Feature Extraction → Model Processing → Text Output
```
...
```
Raw Audio → Resampling → Normalization → Chunking → Model Input
  (any)      (16kHz)      (±1.0)        (30s)       (480k samples)
```

---

## Speech Recognition: Traditional vs Modern
...
- **Wav2Vec2** (Meta) - Self-supervised learning
- **SpeechT5** (Microsoft) - Unified speech-text model
...
- Web-scraped multilingual content
- Diverse domains and speaking styles
- Multiple audio qualities and formats
...
Audio Input (30s)
        ↓
   Mel-Spectrogram (80 channels × 3000 time steps)
        ↓
┌─────────────────┐         ┌─────────────────┐
│     ENCODER     │ ──────→ │     DECODER     │
│                 │         │                 │
│ • 6-12 layers   │         │• 6-12 layers    │
│ • Self-attention│         │• Cross-attention│ 
│ • Audio features│         │• Text generation│
└─────────────────┘         └─────────────────┘
                                      ↓
                            Text Tokens + Timestamps
```

**Encoder:** Processes audio features and creates rich representations

**Decoder:** Generates text tokens using attention over audio features

**Special Tokens:** Language ID, timestamps, no-speech detection
...
- **99 languages** from Afrikaans to Yoruba
- **Automatic language detection**
...
```python
# Language detection
result = model.transcribe("audio.wav", task="detect_language")
...
result = model.transcribe("spanish.wav", task="translate")
...
**Step 3: Chunking (30-second windows)**
```cpp
...
- **Compact representation** (fewer frequency bins)
- **Robust to noise** compared to raw audio
...
3. **Log-scale and normalize** features
...
for (int token : tokens) {
    if (is_timestamp_token(token)) {
        current_timestamp = decode_timestamp(token);
...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  Microphone │ →  │   Whisper   │ →  │ LlmRunner │
│             │    │    STT      │    │     LLM     │
│ "Hello, how │    │             │    │             │
│ are you?"   │    │ Transcribe  │    │ Generate    │
└─────────────┘    └─────────────┘    └─────────────┘
                           │                   │
                           ↓                   ↓
                   "Hello, how           "I'm doing well,
                   are you?"             thank you!"
```
...
for (int step = 0; step < MAX_TOKENS; ++step) {
  auto decoderResult = decoder_->execute("forward", {tokenTensor, encoderOutput_});
  // Extract next token, check EOS, decode to text
}
```
...
std::vector<int64_t> tokens = {START_OF_TRANSCRIPT, 50258, NO_TIMESTAMPS}; // 50258 = <|en|>
std::string result;
```
...
**Expected Result:** "Transcription:  Hello, where is Poland?<|2.00|>"
